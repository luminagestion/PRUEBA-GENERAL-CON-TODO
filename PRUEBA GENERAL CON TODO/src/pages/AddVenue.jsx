...
// Utilidad para cargar array del localStorage
...
export default function AddVenue(){
  ...
  async function handleSubmit(e){
    ...
    try{
      ...
      const newVenue = {
        ...
        lat: markerPos[0],
        lng: markerPos[1],
        capacity: Number(String(form.capacity).replace(',', '.')) || '',
  ...
  return (
    <div className="page add-venue" style={{maxWidth:900, margin:'24px auto', display:'grid', gap:16}}>
      <h2>Agregar venue</h2>
      ...
      <form onSubmit={handleSubmit} className="card" style={cardStyle}>
        <div style={{display:'grid', gap:12, gridTemplateColumns:'1fr 1fr'}}>
          ...
          <label style={{display:'grid', gap:6}}>
            <span>Capacidad</span>
            <input value={form.capacity} onChange={e=>updateField('capacity', e.target.value)} style={inputStyle} />
          </label>

          <label style={{display:'grid', gap:6}}>
            <span>Género predominante</span>
            <select value={form.predominantGenre} onChange={e=>updateField('predominantGenre', e.target.value)} style={inputStyle}>
              <option value="">—</option>
          ...
          <label style={{display:'grid', gap:6}}>
            ...
            <input value={form.whatsapp} onChange={e=>updateField('whatsapp', e.target.value)} placeholder="+54 9 …" style={inputStyle} />
          </label>
        ...
        <div style={{display:'grid', gap:8}}>
          <div style={{fontSize:14, opacity:0.85}}>
            <strong>Busca tu direccion y haz clic sobre ella para agregar</strong>
          </div>
          <div style={{height:360, borderRadius:12, overflow:'hidden', border:'1px solid rgba(255,255,255,0.12)'}}>
            <MapContainer center={[-34.6037, -58.3816]} zoom={12} style={{height:'100%', width:'100%'}}>
              <TileLayer
                attribution="&copy; OpenStreetMap contributors"
                url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
              />
              <ClickToSetMarker onSet={({lat,lng})=>{ updateField('lat', String(lat)); updateField('lng', String(lng)); }} />
          ...
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:12}}>
            <label style={{display:'grid', gap:6}}>
              <span>Latitud*</span>
              <input value={form.lat} onChange={e=>updateField('lat', e.target.value)} required style={inputStyle} />
            </label>
            <label style={{display:'grid', gap:6}}>
              <span>Longitud*</span>
              <input value={form.lng} onChange={e=>updateField('lng', e.target.value)} required style={inputStyle} />
            </label>
          </div>
        </div>
